...
test('should add a valid passenger', async () => {
  ...
  const res = await addPassenger(newP);
...
test('should fail when missing required fields', async () => {
  const res = await addPassenger({ name: '王五' });
  expect(res.status).toBe(400);
...
test('should delete a passenger', async () => {
  ...
  const listRes = await getPassengers({});
  ...
  if (!target) {
     ...
     const res2 = await getPassengers({});
     const target2 = res2.body.passengers.find(p => !p.is_self);
     await deletePassenger(target2.passenger_id);
  } else {
     const res = await deletePassenger(target.passenger_id);
     expect(res.status).toBe(204);
  }
  
  // Verify deleted
  const listRes2 = await getPassengers({});
...
test('should not delete self', async () => {
  ...
  const res = await deletePassenger(self.passenger_id);
